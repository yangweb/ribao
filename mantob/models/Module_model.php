...
class Module_model extends CI_Model {
	...
	public function import_form($dir) {
		...
		foreach ($data as $id => $form) {
			...
			if (!file_exists($file)) {
				...
				.'require FCPATH.\'mantob/core/D_Admin_Form.php\';'.PHP_EOL.PHP_EOL
			...
			if (!file_exists($file)) {
				file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
				.'require FCPATH.\'mantob/core/D_Member_Form.php\';'.PHP_EOL.PHP_EOL
			...
			if (!file_exists($file)) {
				file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
				.'require FCPATH.\'mantob/core/D_Home_Form.php\';'.PHP_EOL.PHP_EOL
	...
	public function install($id, $dir, $siteid, $config, $mydb = 0, $_siteid = 0) {
		...
		if (is_file(FCPATH.$dir.'/config/install.sql')
			...
			$sql_data = explode(';SQL_mantob_EOL', trim(str_replace(array(PHP_EOL, chr(13), chr(10)), 'SQL_mantob_EOL', $_sql)));
