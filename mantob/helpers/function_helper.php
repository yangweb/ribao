...
function om_baidu_map($value, $zoom = 5, $width = 600, $height = 400) {
	...
	drawPoints();
	function drawPoints(){
		var myIcon = new BMap.Icon("' . SITE_URL . 'mantob/statics/images/mak.png", new BMap.Size(27, 45));
...
function dr_show_stars($num, $starthreshold = 4) {
    ...
    for ($i = 3; $i > 0; $i--) {
        ...
        for ($j = 0; $j < $numlevel; $j++) {
            $str.= '<img align="absmiddle" src="' . SITE_URL . 'mantob/statics/images/star_level' . $i . '.gif" ' . $alt . ' />';
        }
    }
