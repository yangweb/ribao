...
function man_to_key() {
	$.post("<?php echo man_url('system/syskey'); ?>", function(data){
		$("#sys_key").val(data);
	});
}
</script>
		...
		<div class="contentList pad-10">
			<div id="cnt_0" style="display:none" class="man_hide">
				...
				<tr>
					...
					<input class="button" type="button" name="button" onclick="man_to_key()" value="<?php echo lang('html-438'); ?>" />
				...
				<tr class="">
                    ...
                    <td>
					<?php echo man_lang('html-478', '<input class="input-text" type="text" name="data[SITE_CONVERT]" value="'.$data['SITE_CONVERT'].'" size="10" />'); ?>
                    </td>
                </tr>
				</table>
			</div>
...
function memcache_test() {
	...
	$.get("<?php echo man_url('system/memcache'); ?>&"+Math.random(),function(data){
		alert(data);
