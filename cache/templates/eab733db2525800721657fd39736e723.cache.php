...
<head>
<meta charset="utf-8">
<title><?php echo man_lang('html-001', SITE_NAME); ?></title>
...
$(function(){
    ...
    if ($('#navigation').height() >= 80) {
        //$("#man_member").hide();
        //is_member_hide = 1;
        $(".man_module_menu").remove();
        $("#man_select_module").show();
    }
    /*
	...
	*/
	$("#man_select_site, #man_select_module").bind({
		'mouseover':function(){
			$(this).addClass("open");
		},
		'mouseout':function(){
	...
	$("#sitelist li").click(function(){
		...
		art.dialog.confirm("<font color=red><b><?php echo lang('html-386'); ?></b></font>", function(){
			// ajax提交验证
			$.ajax({type: "POST",dataType:"json", url: "<?php echo man_url('site/select'); ?>", data: {id: id},
...
function logout(){
	...
	top.location = '<?php echo man_url("login/logout"); ?>';
	return false;
}
function man_get_map() {
	$("#man_backdrop").show();
	$("#modal-map").show();
}
function man_clear_map() {
	$("#man_backdrop").hide();
	$("#modal-map").hide();
}
function man_loading() {
	$('.page-loading').remove();
...
<div class="modal hide" id="modal-map" aria-hidden="true">
    <div class="modal-header">
        <button  onClick="man_clear_map()" class="close" type="button">×</button>
    ...
    <div class="container-fluid">
        ...
        <ul class="main-nav">
            <?php $mark = $i=0;  if (is_array($top)) { $count=count($top);foreach ($top as $id=>$t) { ?>
                <li id="_M_<?php echo $id; ?>" class="<?php if ($i==0) { ?>active<?php }  if ($t['mark']) { ?> man_module_menu<?php } ?>">
           ...
           <!--  <?php if ($mark) { ?>
            <li id="man_select_module" style="display:none">
        ...
        <div class="user">
        	<ul class="icon-nav" id="man_top_nav">
                <li class="dropdown">
                ...
                <li class="dropdown">
                    <a class="dropdown-toggle" href="<?php echo man_url('home/clear'); ?>" title="<?php echo lang('html-573'); ?>" target="right"><i class="icon-trash"></i></a>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" href="<?php echo man_url('home/cache'); ?>" title="<?php echo lang('html-434'); ?>" target="right"><i class="icon-refresh"></i></a>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" href="<?php echo man_url('route/index'); ?>" title="<?php echo lang('html-503'); ?>" target="right"><i class="icon-compass"></i></a>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" href="javascript:;" onClick="man_get_map()" title="<?php echo lang('html-025'); ?>"><i class="icon-sitemap"></i></a>
                </li>
            </ul>
            <div class="dropdown" id="man_member">
                <a data-toggle="dropdown" class="dropdown-toggle" href="javascript:;"><?php echo $admin['username']; ?>&nbsp;<img style="width:27px;height:27px;" src="<?php echo man_avatar($uid); ?>" /></a>
                <ul class="dropdown-menu pull-right">
					...
					<li><div class="man_hr"></div></li>
                    <?php if ($member['adminid'] == 1) { ?>
                    <li><a href="<?php echo man_url('check/index'); ?>" target="right"><i class="icon-zoom-out"></i> <?php echo lang('html-533'); ?></a></li>
                    <?php } ?>
                    <li><a href="javascript:;" onClick="man_get_map()"><i class="icon-sitemap"></i> <?php echo lang('html-025'); ?></a></li>
					<li><a href="<?php echo man_url('home/clear'); ?>" target="right"><i class="icon-trash"></i> <?php echo lang('html-573'); ?></a></li>
					<li><a href="<?php echo man_url('route/index'); ?>" target="right"><i class="icon-compass"></i> <?php echo lang('html-503'); ?></a></li>
					<li><a href="<?php echo man_url('home/cache'); ?>" target="right"><i class="icon-refresh"></i> <?php echo lang('html-434'); ?></a></li>
					<li><div class="man_hr"></div></li>
    ...
    <div id="left">
        <form action="http://www.zan3.com/" method="get" target="_blank" class="search-form" style="margin:9px 0 0;">
            ...
            <div class="search-pane">
                ...
                <button type="submit" id="man_search_submit" title="<?php echo lang('html-146'); ?>"><i class="icon-search"></i></button>
            </div>
        </form>
        <?php echo $left; ?>
    </div>
    <div id="main">
        <iframe name="right" id="rightMain" src="<?php echo man_url('home/main'); ?>" frameborder="false" scrolling="auto" style="border:none; margin-bottom:0px;" width="100%" height="auto" allowtransparency="true"></iframe>
...
<div id="man_backdrop" class="modal-backdrop in hide"></div>
</body>
</html>