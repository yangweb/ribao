...
var man_url = "<?php echo MEMBER_URL; ?>index.php?c=api&m=notice&"+Math.random();
var man_step = 0;
        ...
        <?php if ($member) { ?>
            ...
            <div class="account"><span>账户</span> 
                <ul>
                    ...
                    <li><a href="<?php echo man_member_url('account/avatar'); ?>">上传头像</a></li>
                    <li><a href="<?php echo man_member_url('login/out'); ?>">退出</a></li>
                </ul>
            </div>
        <?php } else { ?>
        	<a class="bt_l" href="<?php echo man_member_url('login/index'); ?>">登录</a>
       		<a class="bt_r" href="<?php echo man_member_url('register/index'); ?>">注册</a>
        <?php } ?>
        </div>
    </div>
</div>
...
<div class="nav_wrapper">
    <div class="nav drop_downs">
        <ul class="nav_list">
            <li><a href="<?php echo man_member_url('home/index'); ?>">首页</a></li>
            <?php if (is_array($menu)) { $count=count($menu);foreach ($menu as $m) { ?>
            <li><a href="<?php if ($m['url']) {  echo $m['url'];  } else {  echo man_member_url($m['uri']);  } ?>" <?php if ($menu_tid==$m['id']) { ?>class="cur" style="color:#D5D5D5"<?php } ?>><?php echo $m['name']; ?></a></li>
			<?php } } ?>
        </ul>
    </div>
</div>
