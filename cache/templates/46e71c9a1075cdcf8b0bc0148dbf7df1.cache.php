...
$(function() {
    $(".select-cat").click(function(){
        ...
        if (action == 'close') {
            $.cookie('man_<?php echo SITE_ID; ?>_<?php echo $this->ci->router->class; ?>_'+catid, null,{path:"/",expires: -1});
            $(this).attr("action", "open");
            $(this).html("[-]");
            for (i=0;i<catids.length ;i++ ) {
                if (catids[i] != catid) {
                    $(".man_catid_"+catids[i]).show();
                }
            }
        } else {
            // 关闭状态存储cookie
            $.cookie('man_<?php echo SITE_ID; ?>_<?php echo $this->ci->router->class; ?>_'+catid, 1);
            $(this).attr("action", "close");
            $(this).html("[+]");
            for (i=0;i<catids.length ;i++ ) {
                if (catids[i] != catid) {
                    $(".man_catid_"+catids[i]).hide();
                }
            }
        }
    });
    $(".select-cat").each(function(){
        var childs = $(this).attr("childs");
        var catid = $(this).attr("catid");
        var cache = $.cookie('man_<?php echo SITE_ID; ?>_<?php echo $this->ci->router->class; ?>_'+catid);
        if (cache) {
            $(this).attr("action", "close");
            $(this).html("[+]");
            var catids= new Array(); //定义一数组
...
<div class="subnav">
	...
	<div class="table-list">
        ...
        <tr>
			...
			<th align="left"><?php if ($this->ci->is_auth('admin/navigator/add')) { ?><a class="add" title="<?php echo lang('add'); ?>" href="<?php echo man_url('navigator/add', array('type' => $type)); ?>"></a><?php } ?></th>
        </tr>
        </thead>
		<tbody>
		<?php echo $list; ?>
		<tr>
            ...
            <?php if ($this->ci->is_auth('navigator/del')) { ?><input type="button" class="button" value="<?php echo lang('del'); ?>" name="button" onClick="$('#action').val('del');return man_confirm_set_all('<?php echo lang('015'); ?>')" />&nbsp;<?php }  if ($this->ci->is_auth('navigator/edit')) { ?><input type="button" class="button" value="<?php echo lang('order'); ?>" name="button" onclick="$('#action').val('order');return man_confirm_set_all('<?php echo lang('015'); ?>')" />&nbsp;<div class="onShow"><?php echo lang('html-054'); ?></div><?php } ?>
			</td>
		</tr>
		</tbody>
		</table>
