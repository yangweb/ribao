...
$(function() {
	$(".select-cat").click(function(){
		...
		if (action == 'close') {
			$.cookie('man_<?php echo SITE_ID; ?>_<?php echo APP_DIR; ?>_<?php echo $this->ci->router->class; ?>_'+catid, null,{path:"/",expires: -1});
			...
			// 关闭状态存储cookie
			$.cookie('man_<?php echo SITE_ID; ?>_<?php echo APP_DIR; ?>_<?php echo $this->ci->router->class; ?>_'+catid, 1);
	...
	$(".select-cat").each(function(){
		var childs = $(this).attr("childs");
		var catid = $(this).attr("catid");
		var cache = $.cookie('man_<?php echo SITE_ID; ?>_<?php echo APP_DIR; ?>_<?php echo $this->ci->router->class; ?>_'+catid);
		if (cache) {
			$(this).attr("action", "close");
			$(this).html("[+]");
			var catids= new Array(); //定义一数组
...
<div class="subnav">
	...
	<div class="table-list">
		...
		<tr>
            ...
            <?php if ($this->ci->is_auth('category/del')) { ?><input type="button" class="button" value="<?php echo lang('del'); ?>" name="button" onClick="$('#action').val('del');return man_confirm_set_all('<?php echo lang('015'); ?>')" />&nbsp;<?php }  if ($this->ci->is_auth('model/edit')) { ?><input type="button" class="button" value="<?php echo lang('order'); ?>" name="button" onclick="$('#action').val('order');return man_confirm_set_all('<?php echo lang('015'); ?>')" />&nbsp;<div class="onShow"><?php echo lang('html-182'); ?></div><?php } ?>
			</td>
		</tr>
		</tbody>
		</table>
