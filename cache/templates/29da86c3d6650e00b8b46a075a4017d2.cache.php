...
<div class="subnav">
	...
	<div class="table-list">
		...
		<tr id="man_row_<?php echo $t['uid']; ?>">
			<td align="right"><input name="ids[]" type="checkbox" class="man_select<?php echo $t['groupid']; ?>" value="<?php echo $t['uid']; ?>" />&nbsp;</td>
			<td align="center"><?php echo $t['uid']; ?></td>
			<td align="left">
            <a onclick="man_dialog_member('<?php echo $t['uid']; ?>')" href="javascript:;">
			<?php if ($t['groupid']==2 && !$t['username']) {  echo get_member_nickname($t['uid']); ?>&nbsp;<?php echo lang('html-586');  } else {  echo man_keyword_highlight($t['username'], $param['keyword']);  } ?>
            </a>
			</td>
			...
			<td align="left"><a <?php if ($this->ci->is_auth('member/admin/experience/index')) { ?>href="<?php echo man_url('member/experience/index',array('uid'=>$t['uid'])); ?>" style="color: blue;text-decoration: underline;font-size: 12px;"<?php } ?>><?php echo $t['experience']; ?></a></td>
			<td align="left"><a <?php if ($this->ci->is_auth('member/admin/score/index')) { ?>href="<?php echo man_url('member/score/index',array('uid'=>$t['uid'])); ?>" style="color: blue;text-decoration: underline;font-size: 12px;"<?php } ?>><?php echo $t['score']; ?></a></td>
			<td align="left"><a <?php if ($this->ci->is_auth('member/admin/pay/index')) { ?>href="<?php echo man_url('member/pay/index',array('uid'=>$t['uid'])); ?>" style="color: blue;text-decoration: underline;font-size: 12px;"<?php } ?>><?php echo $t['money']; ?></a></td>
			<td hide="1" align="left"><?php echo $t['spend']; ?></td>
			<td align="left"><?php echo man_date($t['regtime'], NULL, 'red'); ?></td>
			<td align="left" class="man_option">
			<?php if ($this->ci->is_auth('member/admin/home/edit')) { ?><a <?php if ($t['id']==1) { ?>href="javascript:;" style="color:#999"<?php } else { ?>href="<?php echo man_url('member/home/edit',array('uid'=>$t['uid'])); ?>"<?php } ?>><?php echo lang('edit'); ?></a><?php }  if (MEMBER_OPEN_SPACE && $this->ci->is_auth('member/admin/space/edit')) { ?><a href="<?php echo man_url('member/space/edit',array('uid' => $t['uid'])); ?>"><?php echo lang('html-334'); ?></a><?php }  if ($member['adminid']==1) { ?><a href="<?php echo MEMBER_URL; ?>index.php?c=api&m=ologin&uid=<?php echo $t['uid']; ?>" target="_blank"><?php echo lang('html-703'); ?></a><?php } ?>
			</td>
		</tr> 
		<?php }  } } ?>
		<tr>
			...
			<input type="button" class="button" value="<?php echo lang('del'); ?>" name="option" onClick="$('#action').val('del');man_confirm_set_all('<?php echo lang('015'); ?>', 1)" />
            <?php }  if ($this->ci->is_auth('member/admin/home/edit')) { ?>
			<input type="button" class="button" value="<?php echo lang('html-394'); ?>" name="option" onClick="$('#action').val('update');man_confirm_set_all('<?php echo lang('015'); ?>')" />
            <select name="groupid">
        ...
        <div id="pages"><a><?php echo man_lang('html-346', $param['total']); ?></a><?php echo $_pages; ?></div>
	</div>
</div>
