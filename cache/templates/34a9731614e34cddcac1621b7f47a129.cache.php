...
$(function() {
    ...
    <?php } ?>
    man_remote(<?php echo intval($data['SITE_ATTACH_REMOTE']); ?>);
	man_set_mw_type(<?php echo intval($data['SITE_IMAGE_TYPE']); ?>);
...
function man_set_mw_type(id) {
	$(".man_mw_1").hide();
	$(".man_mw_0").hide();
	$(".man_mw_"+id).show();
}
function man_remote(id) {
	$(".r1").hide();
    $(".r2").hide();
    $(".r3").hide();
	$(".r"+id).show();
}
function man_aliyun_host(val) {
		...
		<div class="contentList pad-10">
			<div id="cnt_0" style="display:none" class="man_hide">
                ...
                <tr>
                    ...
                    <td>
                    <input class="input-text" type="text" name="domain" id="man_domain" value="<?php echo $data['SITE_DOMAIN']; ?>" size="25" />
                    <?php if ($data['SITE_DOMAIN']) { ?>
                    <script>
                        $.get("<?php echo man_url('home/domain', array('domain' => $data['SITE_DOMAIN'])); ?>", function(data){
                            if (data) {
                                $("#man_domian").html(data);
                            } else {
                                $("#man_domian").hide();
                            }
                        });
                    </script>
                    <div id="man_domian" class="onError"></div>
                    <?php } else { ?>
                    <div class="onShow" id="man_domain_tips"><?php echo lang('html-090'); ?></div>
                    <?php } ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                    <input class="input-text" type="text" name="data[SITE_NAME]" id="man_name" value="<?php echo $data['SITE_NAME']; ?>" size="40" />
                    <div class="onShow" id="man_name_tips"><?php echo lang('html-088'); ?></div>
                    </td>
                </tr>
                <tr>
            ...
            <div id="cnt_1" style="display:none" class="man_hide">
                <table width="100%" class="table_form">
                    <tr>
                        <th width="200"><?php echo man_lang('html-290', lang('html-289')); ?>： </th>
                        <td>
                            ...
                            <script>
                                $.get("<?php echo man_url('home/domain', array('domain' => $data['SITE_MOBILE'])); ?>", function(data){
                                    if (data) {
                                        $("#man_mb_domian").html(data);
                                    } else {
                                        $("#man_mb_domian").hide();
                                    }
                                });
                            </script>
                            <div id="man_mb_domian" class="onError"></div>
                            <?php } else { ?>
                            <div class="onShow" id="man_domain_tips"><?php echo lang('html-090'); ?></div>
                            <?php } ?>
                        </td>
			...
			<div id="cnt_3" style="display:none" class="man_hide">
				...
				<tr>
                    ...
                    <input onclick="man_remote(<?php echo $i; ?>)" type="radio" name="data[SITE_ATTACH_REMOTE]" value="<?php echo $i; ?>" <?php if ($data['SITE_ATTACH_REMOTE'] == $i) { ?>checked<?php } ?> />&nbsp;<label><?php echo $t; ?></label>
                    &nbsp;&nbsp;&nbsp;
                    <?php } } ?>
                ...
                <tr class="r3">
                    <th>节点请求地址： </th>
                    <td>
                        <select id="aliyun_host" onchange="man_aliyun_host(this.value)" name="data[remote][3][host]">
			...
			<div id="cnt_4" style="display:none" class="man_hide">
                ...
                <tr>
                    ...
                    <td>
                    <input type="radio" onclick="$('.man_image').show()" name="data[SITE_IMAGE_WATERMARK]" value="1" <?php if ($data['SITE_IMAGE_WATERMARK']) { ?>checked<?php } ?> />&nbsp;<label><?php echo lang('open'); ?></label>
                    &nbsp;&nbsp;&nbsp;
					<input type="radio" onclick="$('.man_image').hide()" name="data[SITE_IMAGE_WATERMARK]" value="0" <?php if (empty($data['SITE_IMAGE_WATERMARK'])) { ?>checked<?php } ?> />&nbsp;<label><?php echo lang('close'); ?></label>
                    </td>
                </tr>
				<tr class="man_image">
				...
				<tr class="man_image">
                    ...
                    <td>
                    <input type="radio" name="data[SITE_IMAGE_TYPE]" value="1" onclick="man_set_mw_type(1)" <?php if ($data['SITE_IMAGE_TYPE']) { ?>checked<?php } ?> />&nbsp;<label><?php echo lang('html-132'); ?></label>
                    &nbsp;&nbsp;&nbsp;
					<input type="radio" name="data[SITE_IMAGE_TYPE]" value="0" onclick="man_set_mw_type(0)" <?php if (empty($data['SITE_IMAGE_TYPE'])) { ?>checked<?php } ?> />&nbsp;<label><?php echo lang('html-133'); ?></label>
                    </td>
                </tr>
				<tr class="man_mw_1 man_image" style="display:none">
				...
				<tr class="man_mw_0 man_image" style="display:none">
				...
				<tr class="man_mw_0 man_image" style="display:none">
                    ...
                    <td>
					<?php echo man_field_input('SITE_IMAGE_COLOR', 'Color', array('option'=>array('value'=>$data['SITE_IMAGE_COLOR']))); ?>
                    </td>
                </tr>
				<tr class="man_mw_0 man_image" style="display:none">
...
function ftp_test() {
	...
	$("#ftptest").val('Loading');
	$.get("<?php echo man_url('api/testftp'); ?>",{rurl:rurl,host:host,port:port,username:username,password:password,pasv:pasv,path:path,mode:mode}, function(data){
		alert(data);
...
function aliyun_test() {
    ...
    $.get("<?php echo man_url('api/aliyuntest'); ?>",{rurl:rurl,id:id,secret:secret,bucket:bucket,host:host}, function(data){
        alert(data);
...
function baidu_test() {
    ...
    $.get("<?php echo man_url('api/baidutest'); ?>",{rurl:rurl,ak:ak,sk:sk,bucket:bucket,host:host}, function(data){
        alert(data);
