...
$(function() {
	...
	$(".man_select_all input").click(function(data){
     ...
     // 关闭搜索
    <?php if ($data['setting']['search']['close']) { ?>man_set_search(1);<?php } ?>
});
...
function man_set_flag(id) {
	art.dialog.open('<?php echo man_url("module/flag"); ?>', {
	...
	<div class="table-list col-tab">
		...
		<div class="contentList pad-10">
			<div id="cnt_0" style="display:none" class="man_hide">
				...
				<tr>
					<th style="font-weight:700;<?php if ($sid>1) { ?>padding-top:30px;<?php } ?>"><?php echo man_strcut($t['SITE_NAME'], 25); ?>： </th>
				...
				<tr class="man_site_<?php echo $sid; ?>" <?php if (!$data['site'][$sid]['use']) { ?>style="display:none"<?php } ?>>
					...
					<div class="onShow"><?php echo man_lang('html-619', $data['dirname']); ?></div>
					</td>
				</tr>
				<tr class="man_site_<?php echo $sid; ?>" <?php if (!$data['site'][$sid]['use']) { ?>style="display:none"<?php } ?>>
					...
					<div class="onShow"><?php echo man_lang('html-620', $data['dirname']); ?></div>
					</td>
				</tr>
				<tr class="man_site_<?php echo $sid; ?>" <?php if (!$data['site'][$sid]['use']) { ?>style="display:none"<?php } ?>>
				...
				<tr class="man_site_<?php echo $sid; ?>" <?php if (!$data['site'][$sid]['use']) { ?>style="display:none"<?php } ?>>
                    ...
                    <?php if ($data['site'][$sid]['domain']) {  if ($data['site'][$sid]['domain'] == SITE_DOMAIN) { ?>
                        <div class="onError"><?php echo man_lang('html-730', $data['site'][$sid]['domain']); ?></div>
                        <?php } else { ?>
                        <script>
                            $.get("<?php echo man_url('home/domain', array('domain' => $data['site'][$sid]['domain'])); ?>", function(data){
                                if (data) {
                                    $("#man_domian_<?php echo $sid; ?>").html(data);
                                } else {
                                    $("#man_domian_<?php echo $sid; ?>").hide();
                                }
                            });
                        </script>
                        <div id="man_domian_<?php echo $sid; ?>" class="onError"></div>
                        <?php }  } else { ?>
            ...
            <div id="cnt_1" style="display:none" class="man_hide">
                ...
                <tr>
                    ...
                    <td><a href="javascript:;" onclick="man_admin_rule('<?php echo $t['id']; ?>', '<?php echo man_url("module/role", array("dir" => $data['dirname'], "id" => $t['id'])); ?>', '<?php echo $t['name']; ?>')" class="blue">[<?php echo lang('028'); ?>]</a>
					<div id="man_status_<?php echo $t['id']; ?>" class="onShow"></div>
					</td>
                </tr>
                <?php }  } } ?>
				</table>
            ...
            <div id="cnt_2" style="display:none" class="man_hide">
                ...
                <tr>
					...
					<label><?php echo lang('html-511'); ?></label>&nbsp;<input class="man_use" type="checkbox" />
					</td>
				</tr>
				...
				<tr>
					...
					<td align="left">&nbsp;</td>
					<td align="left" style="padding-left:40px"><?php echo $level['name']; ?>&nbsp;&nbsp;<?php echo man_show_stars($level['stars']); ?></td>
					<td align="left">
					<label><?php echo lang('html-511'); ?></label>&nbsp;<input class="man_use" name="data[setting][member][<?php echo $id; ?>]" <?php if ($data['setting']['member'][$id]) { ?>checked<?php } ?> value=1 type="checkbox" />
					</td>
				</tr>
				<?php } }  } else { ?>
				<tr>
			...
			<div id="cnt_3" style="display:none" class="man_hide">
                ...
                <tr <?php if ($i==9) { ?>class=""<?php } ?>>
                    ...
                    <?php $return_group = $this->list_tag("action=cache name=MEMBER.group  return=group"); if ($return_group) extract($return_group); $count_group=count($return_group); if (is_array($return_group)) { foreach ($return_group as $key_group=>$group) {  if ($group['id'] > 2) {  if (is_array($group['level'])) { $count=count($group['level']);foreach ($group['level'] as $level) {  $iid=$group['id'].'_'.$level['id']; ?>
                            <input name="data[setting][flag][<?php echo $i; ?>][<?php echo $iid; ?>]" class="man_flag_<?php echo $i; ?>" iid="<?php echo $iid; ?>" id="man_flag_<?php echo $i; ?>_<?php echo $iid; ?>" type="hidden" value="<?php echo intval($data['setting']['flag'][$i][$iid]); ?>" />
                        <?php } }  }  } } ?>
                    <a href="javascript:;" onclick="man_set_flag('<?php echo $i; ?>')" style="color:#06F">[<?php echo lang('html-175'); ?>]</a>
					<div id="man_status_<?php echo $i; ?>" class="onShow"></div>
                    </td>
                </tr>
                <?php } ?>
				</table>
            ...
            <div id="cnt_6" style="display:none" class="man_hide">
                ...
                <tr>
                    ...
                    <td>
                        <input type="radio" name="data[setting][search][close]" <?php if ($data['setting']['search']['close']) { ?>checked<?php } ?> onclick="man_set_search(1)" value="1" />&nbsp;<label><?php echo lang('yes'); ?></label>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="radio" name="data[setting][search][close]" <?php if (!$data['setting']['search']['close']) { ?>checked<?php } ?> onclick="man_set_search(0)" value="0" />&nbsp;<label><?php echo lang('no'); ?></label>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
